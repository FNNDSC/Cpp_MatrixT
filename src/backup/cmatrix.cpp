...
// NAME
//
//      cmatrix.cpp     $Id: cmatrix.cpp,v 1.4 2004/06/28 20:17:34 rudolph Exp $
//
// DESCRIPTION
//
...
template <typename _CMDATA>
CMatrix<_CMDATA>::CMatrix(CMatrix<_CMDATA> & z)
{
    //
    // DESC
...
CMatrix<GSL_complex>::CMatrix(CMatrix<GSL_complex> & z)
{
    //
    // DESC
    //  Copy constructor - simple pointer based.
...
CMatrix<GSL_complex>::fft1D(
    ...
    if(!b_MKLwrap) {
        ...
        double*                         pv_real         = new double[n];
        double*                         pv_imag         = new double[n];
        double*                         pv_wave         = new double[3*n];
        int                             isign           = -1;
       ...
       // Fill the pv_real and pv_imag vectors
        for (i = 0; i < n; i++) {
            pv_real[i]          = GSL_REAL(_mval(0, i));
            pv_imag[i]          = GSL_IMAG(_mval(0, i));
        }

        // Perform the required operation
        //zfft1dc(pv_real, pv_imag, n, 0, pv_wave);
        //zfft1dc(pv_real, pv_imag, n, isign, pv_wave);

        // Pack the results in the return CMatrix<GSL_complex>
        for (i = 0; i < n; i++) {
            GSL_complex     z_data(0, 0);
            GSL_SET_COMPLEX(&z_data, pv_real[i], pv_imag[i]);
            _mval(0, i)             = z_data;
        }
...
CMatrix<GSL_complex>::fft2D(
    ...
    if(!b_MKLwrap) {
        ...
        double*                         pv_real         = new double[n*m];
        double*                         pv_imag         = new double[n*m];
        int                             isign           = -1;
        switch(e_direction) {
            case e_forward:
                isign   = -1;
        ...
        for (i = 0; i<m; i++) {
            for(j = 0; j<n; j++) {
                pv_real[n*i+j]          = GSL_REAL(_mval(i, j));
                pv_imag[n*i+j]          = GSL_IMAG(_mval(i, j));
            }
        }

        // Perform the required operation
        //zfft2dc(pv_real, pv_imag, m, n, isign);

        // Pack the results in the return CMatrix<GSL_complex>
        for (i=0; i<m; i++) {
            for(j=0; j<n; j++) {
                GSL_complex     z_data(0, 0);
                GSL_SET_COMPLEX(&z_data, pv_real[n*i+j], pv_imag[n*i+j]);
                _mval(i, j)             = z_data;
            }
        }
...
CMatrix<GSL_complex_float>::CMatrix(CMatrix<GSL_complex_float> & z)
{
    //
    // DESC
    //  Copy constructor - simple pointer based.
...
CMatrix<GSL_complex_float>::fft1D(
    ...
    if(!b_MKLwrap) {
        ...
        float*                         pv_real         = new float[n];
        float*                         pv_imag         = new float[n];
        float*                         pv_wave         = new float[3*n];
        int                            isign           = -1;
       ...
       // Fill the pv_real and pv_imag vectors
        ...
        // Perform the required operation
        //cfft1dc(pv_real, pv_imag, n, 0, pv_wave);
        //cfft1dc(pv_real, pv_imag, n, isign, pv_wave);
...
CMatrix<GSL_complex_float>::fft2D(
    ...
    if(!b_MKLwrap) {
        ...
        float*                         pv_real         = new float[n*m];
        float*                         pv_imag         = new float[n*m];
        int                            isign           = -1;
        switch(e_direction) {
            case e_forward:
                isign   = -1;
        ...
        // Perform the required operation
        //cfft2dc(pv_real, pv_imag, m, n, isign);
