...
string		Gstr_version	="$Id: TSfftf.cpp,v 1.1.1.1 2004/06/16 15:53:07 rudolph Exp $";
...
void
dataSet1D_create(
        CMatrix<GSL_complex_float>*&  pzM_input,
        int                     columns) {
    //
    // ARGS
    ...
    if(!pzM_input->compatible(1, columns)) {
        delete pzM_input;
        pzM_input       = new CMatrix<GSL_complex_float>(1, columns);
    }
...
dataSet2D_create(
    ...
    if(!pzM_input->compatible(rows, columns)) {
        delete pzM_input;
        pzM_input       = new CMatrix<GSL_complex_float>(rows, columns);
    }
...
//    CVol<GSL_complex_float>  Vol(rows, columns, slices);
...
//    trials      = 2;
    ...
    CMatrix<GSL_complex_float>*       pzM_input       = new CMatrix<GSL_complex_float>(1, 1);
    CMatrix<GSL_complex_float>*       pzM_output      = new CMatrix<GSL_complex_float>(1, 1);

    CVol<GSL_complex_float>*		pVl_input	= new CVol<GSL_complex_float>(1, 1, 1);
    CVol<GSL_complex_float>*		pVl_output	= new CVol<GSL_complex_float>(1, 1, 1);

    CVol<GSL_complex_float>*            pVl_test1       = new CVol<GSL_complex_float>(rows, columns, slices);
    CVol<GSL_complex_float>*            pVl_test2       = new CVol<GSL_complex_float>(*pVl_test1);

    CMatrix<GSL_complex_float>**      ppzM_input      = NULL;
    CMatrix<GSL_complex_float>**      ppzM_output     = NULL;
	...
	delete 			pVl_input;
	delete			pVl_output;
	pVl_input		= new CVol<GSL_complex_float>(rows, columns, slices);
	pVl_output		= new CVol<GSL_complex_float>(rows, columns, slices);
        dataSet3D_create(pVl_input, rows, columns, slices, true);
...
//    pVl_input->print("Input volume");
	...
	if(b_2D) {
	    ...
	    //pzM_output->fft2D(e_forward, b_MKLwrap);
            //*pzM_output = pzM_input->fftshift(false);
            *pzM_output = pzM_input->fftshift(false);
        }
	if(b_3D) {
...
//            pVl_output->copy(*pVl_input);
	    //pVl_output->fft3D(e_inverse, b_MKLwrap);
            //*pVl_output  = pVl_input->fftshift(false);
           pVl_output->fftshift(true);
	}
    }
